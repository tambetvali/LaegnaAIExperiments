...
This is **API** - the *full initial task* - and **specification** - this can accompany the *API* to contain *pre-generated automatic implementation* tasks for backwards- and forewards- **compability**.

The *API* part can be used inside other files to be aware of this *entry point*, because it's *not good to paste long body of one document into another* (it creates exponentiality into AI resource use and might disable your task, so always have an entry point or "*API*" for your specifications); *API* and *Spec* together can be used mostly in local context for specific task or part to keep the implementation consistent and not allow more change in the actual **code body**, than allowed through *specification*: *we do not want to make the code wholly static in AI realm*, but rather be *consistent that refactoring follows linear process* (if some *function names* or *bodies* change, for example if you switch the WYSIWYG editor, it's not a break into your process, because the change is rather **linear**, local and conscievable; if you don't have *Spec* at all - the change might need **architectural changes** and make your code **highly volatile**, which means you are *not able to refactor the clients and users*; this will be **continuously our coding model**).
